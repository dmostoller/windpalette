...
export default function ColorSwatch({
  ...
  const textColor = parseInt(shade.toString()) <= 300 ? darkColor || "black" : "white";
  return (
    <button
      onClick={onClick}
    ...
    >
      <div className="w-full h-full transition-colors" style={{ backgroundColor: color }} />
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        <span className="text-xs font-medium drop-shadow-md" style={{ color: textColor }}>
          {shade}
        </span>
        {showHex && (
          <span className="text-[0.6rem] drop-shadow-md mt-0.5" style={{ color: textColor }}>
            {color}
          </span>
        )}
        {isLocked !== undefined &&
